...
export default function PomodoroPage() {
  ...
  const [availableAssignments, setAvailableAssignments] =
    useState(assignmentsData); // Available assignment tasks
  ...
  const addTask = (assignment: any) => {
    ...
    setAvailableAssignments((prev) =>
      prev.filter((asg) => asg.id !== assignment.id)
    );
  };
  ...
  return (
    <div className="pomodoro-container p-6 text-black text-center bg-cyan-900">
      {/* Face Detection component, runs when timer is active */}
      {timerOn && <FaceDetection onFocusChange={setIsFocused} />}

      {/* Timer and tasks section */}
      <div
        className={`timer-tasks-container p-8 rounded-lg shadow-lg ${
          timerOn ? "bg-blue-100" : "bg-white"
        }`}
      >
        ...
        {/* Timer */}
        <div
          className={`timer text-8xl mb-8 ${
            timerOn ? "bg-blue-100 p-4 rounded-lg" : ""
          }`}
        >
          {formatTime(time)}
        </div>
            ...
            {tasks.map((task) => (
              <li
                ...
                className={`relative p-4 rounded-lg shadow-lg ${
                  timerOn ? "bg-yellow-200" : "bg-gray-100"
                }`}
              >
                <div>
                    ...
                    {task.questions.map((question) => (
                      <li key={question.id} className="flex items-center">
                        <input
                          ...
                          onChange={() =>
                            toggleQuestionCompletion(task.id, question.id)
                          }
                          className="mr-2"
                        />
                        <span
        ...
        <div className="task-toggle-container mt-12 p-8 rounded-lg shadow-lg bg-white">
          ...
          {showUpcomingTasks ? (
            <div className="assignments mt-12 mx-auto max-w-3xl">
              ...
              {["Due Today", "Due This Week", "Due Later", "Past Due"].map(
                (category) => (
                        ...
                        <ul className="space-y-4">
                          {categorizedAssignments[category]
                            .slice(
                              0,
                              showMoreAssignments
                                ? categorizedAssignments[category].length
                                : 3
                            )
                            .map((assignment) => (
                              <li
